/**
 * providers/deepseek.js — Disor Bot v7.0 "Ironclad"
 * ═══════════════════════════════════════════════════════════
...
const { DEEPSEEK_TOKEN, RAILWAY_URL, POW_PROXY_TELEGRAM, DEFAULT_POW_PROVIDER } = require('../config');
...
async function _new_ds_session(token = DEEPSEEK_TOKEN) {
    ...
    if (!sid) throw new Error(`Bad session_response: ${JSON.stringify(resp.data)}`);
    return sid;
}
